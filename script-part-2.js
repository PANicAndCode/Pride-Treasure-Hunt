...
function saveLocalBoard(){
  ...
  board[teamKey] = {
    ...
    completedAt: finishedAtForState(state),
    completionTimeMs: completionTimeMsForState(state),
...
function compareBoardRows(a, b){
  ...
  return (b.found - a.found)
    ...
    || (toMillis(a.startedAt) - toMillis(b.startedAt))
    || a.teamName.localeCompare(b.teamName);
}
...
function finishedPlacementRows(){
  return boardRows().filter(row => row.finished).sort((a, b) => (a.completionTimeMs - b.completionTimeMs) || (a.completedAt - b.completedAt) || a.teamName.localeCompare(b.teamName));
}
...
function finalEggReadyMessage(){
  return "Your final clue is unlocked. Find the final checkpoint and scan its QR code to stop your clock.";
}
...
function normalizeRemoteProgress(data){
  ...
  return {
    ...
    completedAt: data.finished_at ?? 0,
...
function presetStorageMissing(error){
  return error?.code === "42P01"
    || error?.code === "PGRST205"
    || /game_presets_pride_hunt/i.test(String(error?.message || ""));
}
...
function enableLocalSyncFallback(message = "Run treasure_hunt_setup.sql once to enable live Pride Hunt sync."){
  supabaseReady = false;
...
function describeSharedProgressChange(team, previous, next){
  ...
  if (!previous) return `${name} just joined the Pride race.`;
  if (!previous.finished && next.finished) return `${name} just finished in ${formatDuration(completionTimeMsForState(next))}.`;
...
async function fetchLeaderboard(){
  if (!supabaseReady) return;
  try {
    const { data, error } = await supabaseClient.from(LEADERBOARD_TABLE).select("*");
    if (error){
      if (sharedSyncStorageMissing(error)) {
        enableLocalSyncFallback("Run treasure_hunt_setup.sql once to enable live Pride Hunt sync.");
        return;
      }
      console.error(error);
...
async function fetchAllRemoteProgress(){
  if (!supabaseReady) return;
  try {
    const { data, error } = await supabaseClient.from(PROGRESS_TABLE).select("*");
    if (error){
      if (sharedSyncStorageMissing(error)) {
        enableLocalSyncFallback("Run treasure_hunt_setup.sql once to enable live Pride Hunt sync.");
        return;
...
function subscribeLeaderboard(){
  ...
  supabaseClient.channel("leaderboard-live")
    .on("postgres_changes", { event: "*", schema: "public", table: LEADERBOARD_TABLE }, payload => {
...
function subscribeTeamProgress(){
  ...
  supabaseClient.channel("team-progress-live")
    .on("postgres_changes", { event: "*", schema: "public", table: PROGRESS_TABLE }, async payload => {
      const row = payload.new || payload.old;
      if (!row || !row.team_id) return;
...
async function loadRemoteProgress(team){
  ...
  try {
    const { data, error } = await supabaseClient.from(PROGRESS_TABLE).select("*").eq("team_id", team).maybeSingle();
    if (error){
      if (sharedSyncStorageMissing(error)) {
        enableLocalSyncFallback("Run treasure_hunt_setup.sql once to enable live Pride Hunt sync.");
        return null;
...
async function pushRemoteProgress(){
  ...
  const payload = {
    ...
    finished_at: finishedAtForState(state),
    completion_time_ms: completionTimeMsForState(state),
...
async function pushLeaderboard(){
  ...
  const { error } = await supabaseClient.from(LEADERBOARD_TABLE).upsert(payload, { onConflict: "team_id" });
  if (error) {
    console.error(error);
...
function cacheBoardLocally(team, targetState){
  ...
  localBoard[team] = {
    ...
    completedAt: finishedAtForState(targetState),
    completionTimeMs: completionTimeMsForState(targetState),
  ...
  liveBoardCache[team] = {
    ...
    finished_at: finishedAtForState(targetState),
    completion_time_ms: completionTimeMsForState(targetState),
...
async function upsertSharedTeamState(team, targetState){
  ...
  if (!supabaseReady) return;
  const progressResult = await supabaseClient.from(PROGRESS_TABLE).upsert({
    team_id: team,
    team_name: targetState.teamName,
  ...
  const boardResult = await supabaseClient.from(LEADERBOARD_TABLE).upsert({
    team_id: team,
    team_name: targetState.teamName,
    found: targetState.completed.length,
    finished: targetState.finished,
...
function renderTop(){
  ...
  if (el("raceClockValue")) el("raceClockValue").textContent = formatDuration(elapsedTimeMsForState(state));
  if (el("raceClockMeta")) {
    el("raceClockMeta").textContent = state.finished
      ? `Clock stopped at ${formatDuration(completionTimeMsForState(state))}`
      : (state.startedAt ? "Your timer is live until the final QR." : "Your timer starts when your team goes live.");
  }
  ...
  rows.forEach((row, i) => {
    ...
    div.innerHTML = `
      ...
      <div class="leaderRight">
        <strong>${row.finished ? formatDuration(row.completionTimeMs) : formatDuration(row.elapsedMs)}</strong>
        <div class="small">${row.finished ? "finish time" : "live clock"}</div>
      </div>`;
