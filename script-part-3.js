function applyProgressAdvance(team, targetState, scannedValue){
  ...
  if (targetState.progressIndex >= teamTotal(team)) {
    ...
    targetState.completedAt = Date.now();
    targetState.completionTimeMs = Math.max(0, Number(targetState.completedAt || 0) - Number(targetState.startedAt || 0));
    targetState.lastUpdatedAt = targetState.completedAt;
...
async function unlockToken(token, options = {}){
  ...
  if (!expected){
    ...
    if (!quiet) setFeedback(message, "warn");
    setScanInsight("This device has already cleared the full race. Ask an admin if you need to clear the saved team on this phone.");
...
function renderFinalEggCard(){
  ...
  if (isOnFinalClue(state, teamKey)){
    ...
    copy.textContent = "Find the final checkpoint and scan its QR code to stop your clock. The fastest completed time wins.";
...
async function adminGrantNext(){
  ...
  if (isReadyForVictory(targetState, team)) {
    targetState.finished = true;
    if (!targetState.completedAt) targetState.completedAt = Date.now();
    if (!targetState.completionTimeMs) targetState.completionTimeMs = Math.max(0, Number(targetState.completedAt || 0) - Number(targetState.startedAt || 0));
    targetState.lastUpdatedAt = Number(targetState.completedAt || Date.now());
    result = { status: "finished", message: `${label} finished the Pride race.` };
  } else {
    const expected = expectedTokenForState(targetState, team);
    if (!expected){
      el("adminPanelFeedback").textContent = "That team has already finished the Pride race.";
      return;
    }
  ...
  if (targetState.finished) {
    ...
    el("adminPanelFeedback").textContent = `${label} finished the Pride race in ${placementLabel(place)} with a time of ${formatDuration(completionTimeMsForState(targetState))} and won ${placementPrizeText(place)}.`;
  } else if (result.status === "ready-final-egg") {
    el("adminPanelFeedback").textContent = `Granted ${label} the final clue. They still need to scan the final checkpoint QR code to stop their clock.`;
  } else {
...
async function adminResetAll(){
  ...
  if (supabaseReady) {
    const progressDelete = await supabaseClient.from(PROGRESS_TABLE).delete().neq("team_id", SHARED_SETTINGS_TEAM_ID);
    const boardDelete = await supabaseClient.from(LEADERBOARD_TABLE).delete().neq("team_id", "__never__");
